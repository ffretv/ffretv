...
For those needing wireless use, 1080x in any form from typical mythtv sources is generally too much data, I recommend only using 720p max for the output resolution option on wireless connected frontends. Perhaps if I had used a high end wireless router I could recommend different, but I doubt it. I think it would take mobile mp4 profile media sources and the new wireless to be stable with this. 
